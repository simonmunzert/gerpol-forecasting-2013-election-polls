...
df.final <- df.final[df.final$institute!="insa",]
 save(file="data/df.final.RData",list="df.final")


### Datensatzbearbeitung

load("data/df.final.RData")
...
save(file="data/data.forecast.RData",list="data.forecast")


data.forecast$date <- as.Date(data.forecast$date)
write.dta(data.forecast, file="data/data_wahlrecht.dta", version=10, convert.dates = TRUE)
